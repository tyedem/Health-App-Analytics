...
For the datasets with < 33 distinct IDs, queries were run to obtain list of these IDs and to confirm whether there are IDs that are not found in the full list of 33 users. No discrepancies identified.
...
-- JOIN count with daily_activity.Id with heartrate_seconds.Id

--CREATE VIEW `linear-freehold-354220.bellabeat.id_view1` AS
SELECT DISTINCT(daily_activity.Id)
FROM `linear-freehold-354220.bellabeat.daily_activity` AS daily_activity
INNER JOIN `linear-freehold-354220.bellabeat.heartrate_seconds` AS heartrate_seconds ON daily_activity.Id = heartrate_seconds.Id
ORDER BY daily_activity.Id;
...
SELECT COUNT(DISTINCT(Id)) AS IDCount
FROM `linear-freehold-354220.bellabeat.hourly_steps`;
...
SELECT DISTINCT(daily_activity.Id)
FROM `linear-freehold-354220.bellabeat.daily_activity` AS daily_activity
INNER JOIN `linear-freehold-354220.bellabeat.minute_sleep` AS minute_sleep ON daily_activity.Id = minute_sleep.Id
ORDER BY daily_activity.Id;
...
SELECT DISTINCT(daily_activity.Id)
FROM `linear-freehold-354220.bellabeat.daily_activity` AS daily_activity
INNER JOIN `linear-freehold-354220.bellabeat.sleep_day` as sleep_day ON daily_activity.Id = sleep_day.Id
ORDER BY daily_activity.Id;
...
SELECT DISTINCT(daily_activity.Id)
FROM `linear-freehold-354220.bellabeat.daily_activity` AS daily_activity
INNER JOIN `linear-freehold-354220.bellabeat.weight_log_info` as weight_log_info ON daily_activity.Id = weight_log_info.Id
ORDER BY daily_activity.Id;
