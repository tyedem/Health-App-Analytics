...
Description: These MIN/MAX datetime queries were conducted during the Prepare phase of the project to validate whether the datasets are reliable, original, comprehensive, current and cited (ROCCC):

Results: Most datasets have MIN(12APR2022) with MAX(12MAY2022). Only datasets 10, 14, and 16 fall outside of this date range by +/- 1 day. Exact date ranges of these datasets are captured below.
...
-- MIN/MAX datetime values
SELECT 
    MIN(ActivityHour) AS MinDate, 
    MAX(ActivityHour) AS MaxDate
FROM 
...
-- MIN/MAX datetime values
SELECT 
    MIN(ActivityMinute) AS MinDate, 
    MAX(ActivityMinute) AS MaxDate
FROM 
...
-- MIN/MAX datetime values
SELECT 
    MIN(SleepDay) as MinDate, 
    MAX(SleepDay) AS MaxDate
FROM 
